...
sw.init = function () {
    ...
    shader = sw.Shader.fromSource({
        [sw.Shader.Stage.Vertex]:
`#version 330 core
...
void main() {
    fragment_color = vec4(vec3(snoise(vec3(uv_.x * 3, uv_.y * 3, elapsed)) * 0.5 + 0.5), 1);
}`
    });
...
sw.update = function (delta: number) {
    ...
    sb.projectionMatrix = m;

    sb.begin();
    shader.bind();
    shader.getUniform("elapsed").set(elapsed);
